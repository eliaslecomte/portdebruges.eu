...
export function formatSuperforecast(data: superforecastsResponse) {
  ...
  // TODO: move
  const todayStart = toZonedTime(Date.now(), 'Europe/Brussels');
  todayStart.setHours(7);
  todayStart.setMinutes(0);
  todayStart.setSeconds(0);
  todayStart.setMilliseconds(0);
  const todayEnd = toZonedTime(Date.now(), 'Europe/Brussels');
  ...
  return {
    today: mappedData.filter((item) => {
      const dateAsNumber = toZonedTime(item.date, 'Europe/Brussels');
